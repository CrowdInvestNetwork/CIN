...
def height():
    ...
    if 'height' not in resp:
        print ('Unexpected response, make sure CrowdInvestNetworkd is running',
               resp)
        sys.exit(-1)
    else:
        return resp['height']
...
def rpc(method, params={}):
    ...
    if 'result' not in resp:
        print ('Unexpected response, make sure CrowdInvestNetworkd is running with block explorer enabled'
               , resp)
        sys.exit(-1)
    else:
        return resp['result']
