...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/CrowdInvestNetwork ]; then
            read -r -p "${1:-CrowdInvestNetwork directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old CrowdInvestNetwork directory" && echo
                    rm -rf "$PWD"/CrowdInvestNetwork
                    ;;
                *)
                    _fail "CrowdInvestNetwork directory already exists. Aborting..."
                    ;;
            esac
        fi
...
_build_CrowdInvestNetwork() {
    _note "Building CrowdInvestNetwork from source (this might take a while)..."
    if [ -d build ]; then
...
_note "CrowdInvestNetwork Multi_Installer v1.0 (pepperoni)"
_colorize green " ██████╗██╗███╗   ██╗\n██╔════╝██║████╗  ██║\n██║     ██║██╔██╗ ██║\n██║     ██║██║╚██╗██║\n╚██████╗██║██║ ╚████║\n ╚═════╝╚═╝  ╚═╝ ╚══╝\n" && echo
